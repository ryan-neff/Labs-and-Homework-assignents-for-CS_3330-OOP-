   ...
   */
   ...
   public boolean dropItem(Item item){
	   for(Item bagItem : items){     										 //for each item in the bag..
		   if( item != null && bagItem.getName().equals(item.getName())){   //..if the item is not null and the name if the item matches the search item..
   ...
   public Item getItem(String name){
	   for(Item item : items){             //for every item in the array list
		   if(item.getName().equals(name)){   //..if the item is found in the array list
